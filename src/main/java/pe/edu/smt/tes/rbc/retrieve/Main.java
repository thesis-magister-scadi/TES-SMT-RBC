...
public class Main {
	...
	public static void main(String args[]) throws Exception {
		...
		double[] prom = new double[split];
		
		int iter = 10;
		
		for(int ts=0; ts<iter; ts++) {
			System.out.println("Train Set -> "+trainSet[ts]);
			System.out.println("Test Set -> "+testSet[ts]);
			String[] train = trainSet[ts].split(",");
			String[] test = testSet[ts].split(",");
			...
			System.out.println("respuestas reales -> "+Arrays.toString(labelReals));
			
			for(RBCCaso caso:testCases) {
				//caso.setLabel(RBCKnn.KnnEuclidianDistance(trainCases, caso, 5));
				//caso.setLabel(RBCKnn.KnnCommonFeatures(trainCases, caso, 5));
				caso.setLabel(RBCReuse.reuse(RBCRetrieve.retrieve(trainCases, caso), 5));
				trainCases.add(caso);
			}	
			...
			error = testCases.size()-correct;
			System.out.println("Correct "+correct+" Error "+error);
			
			prom[ts] = (((double)correct*100)/(double)test.length);
			System.out.println("Porcentaje clasificaicon -> "+prom[ts]);
		}
		...
		System.out.println("Promedio general: "+(total/((double) iter)));
		
		/*
		String[] train = indexTrain.split(",");
		String[] test = indexTest.split(",");
		...
		error = testCases.size()-correct;
		System.out.println("Correct "+correct+" Error "+error);
		System.out.println(correct*100/test.length);
		*/

		/*
		List<RBCCaso> cases = RBCDataSource.getDataSet("/home/luis/Documents/dataset/SCADI.csv", ",");
		RBCCaso caso = cases.get(66);
		List<RBCCaso> newCases = cases.stream().filter(x -> x.getId() != 67).collect(Collectors.toList());
		
		System.out.println("Real label -> "+caso.getLabel());
		System.out.println("Knn euclidian label -> "+RBCKnn.KnnEuclidianDistance(newCases, caso, 5));
		System.out.println();
		System.out.println("Knn common label -> "+RBCKnn.KnnCommonFeatures(newCases, caso, 5));
		*/
		
		/*
		newCases.stream().forEach(e -> {
			e.setDistance(RBCKnn.calculateEuclidianDistance(e.getFeatures(), caso.getFeatures()));
		});
		 */
		
		//Collections.sort(newCases, (p, q) -> p.getEuclidianDistance().compareTo(q.getEuclidianDistance()));
		//Collections.reverse(newCases);
		/*
		newCases.forEach(x -> {
			System.out.println("id: " + x.getId() + ", distance: " + x.getDistance());
			System.out.println("Etiqueta Propuesta -> "+x.getLabel());
		});
		*/
