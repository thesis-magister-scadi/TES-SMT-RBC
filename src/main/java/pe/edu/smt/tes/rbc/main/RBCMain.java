...
public class RBCMain {
	...
	public static double testCrossValidation(int[] cromosome) throws FileNotFoundException, IOException {
		...
		for(int i=0; i<itr; i++) {
			if(flag) 
				accuracies[i] = run(cromosome, trainSet[i], testSet[i], 3);
			else 
				accuracies[i] = run(trainSet[i], testSet[i], 3);
		}
		System.out.println(accurary(accuracies));
		return accurary(accuracies);
	...
	private static double run(String trainSet, String testSet, int neighbors) throws FileNotFoundException, IOException {
		List<RBCCaso> cases = RBCDataSource.getDataSet("/home/luis/Documents/dataset/SCADI_BINARY.csv", ",");
	...
	private static double run(int[] cromosome, String trainSet, String testSet, int neighbors) throws FileNotFoundException, IOException {
		//List<RBCCaso> cases = RBCDataSource.getDataSetWithFilterFeatures("/home/luis/Documents/dataset/SCADI_BINARY.csv", ",",cromosome);
